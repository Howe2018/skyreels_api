...
# 1. Text2Video Sync Submit/Query
# This test submits a task and performs manual polling until completion.

def main():
    parser = argparse.ArgumentParser(description="Text2Video Sync Submit/Query Test")
    ...
    try:
        print(f"Submitting text2video task (sync) with prompt: {args.prompt} (mode: {args.mode}, sound: {args.sound})")
        ...
        while True:
            task = client.get_text2video_task(resp.task_id)
